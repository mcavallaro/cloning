...
int main(int argc, char *argv[]) {
  ...
  snprintf(file_name, 120, "%sDens_mem_TASEP_clon_%s_%s_%s_%s_%s_%s.txt", argv[7],argv[1],argv[2], argv[3], argv[4], argv[5], argv[6]);
  ...
  vector<double> densityprofile(dim_chain,0);
  ...
  for  (i =0; i< DIM_ENSEMBLE; i++) {
      ...
      times[i] = 1 + time_increment_f(&system[0], i, br, r,  times);
  }
  ...
  while (DT < T){
    ...
    if (DT > T-100){
      fill(densityprofile.begin(), densityprofile.end(), 0);
      for (int l=0; l<dim_chain; l++){
        for (int k=0; k<DIM_ENSEMBLE; k++){
          densityprofile[l] = densityprofile[l] + system[k].node[l];
        }
        fprintf(file, "%f ", densityprofile[l]/DIM_ENSEMBLE);
      }
      fprintf(file,"\n");
    }
  }
